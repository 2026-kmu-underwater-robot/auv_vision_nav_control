...
class YoloBuoyDetector(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("detection_topic", "/vision/buoy_detection_2d")
        ...
        self.declare_parameter("publish_all_targets", True)
        ...
        self.publish_all_targets = bool(
            self.get_parameter("publish_all_targets").value
        )
        ...
        self.detection_pub = self.create_publisher(BuoyDetection2D, self.detection_topic, 10)

        self.get_logger().info(f"Subscribing: {self.image_topic}")
        self.get_logger().info(f"Publishing 2D detections: {self.detection_topic}")
        ...
        self.get_logger().info(
            ...
            f"publish_all_targets={self.publish_all_targets}"
        )
    ...
    def on_image(self, msg: CompressedImage) -> None:
        try:
            ...
            self.get_logger().error(
                f"YOLO image callback failed: {type(exc).__name__}: {exc}",
                throttle_duration_sec=2.0,
            )

    def _process_image(self, msg: CompressedImage) -> None:
        ...
        if self.publish_all_targets:
            published_detections = self._all_matching_detections(all_detections)
        elif self.publish_per_class:
            published_detections = self._best_detection_per_class(all_detections)

        if published_detections:
            for index, published_detection in enumerate(published_detections):
                self._publish_detection(
                    msg, published_detection, image_width, image_height,
                    len(published_detections), index,
                )
        else:
            self._publish_detection(msg, None, image_width, image_height, 0, 0)
    ...
    def _all_matching_detections(
        self,
        all_detections: list[Tuple[int, float, float, float, float, float, int, int, int, int]],
    ) -> list[Tuple[int, float, float, float, float, float]]:
        """Publish every matching box so same-class buoys can be tracked independently."""
        detections = [
            (class_id, confidence, center_x, center_y, width, height)
            for class_id, confidence, center_x, center_y, width, height, *_ in all_detections
            if self._class_matches(class_id)
        ]
        # Stable ordering makes detection_index deterministic within a frame.
        return sorted(detections, key=lambda item: (item[0], item[2], item[3]))
    ...
    def _publish_detection(
        ...
        out.detections_in_frame = detections_in_frame
        out.detection_index = detection_index
